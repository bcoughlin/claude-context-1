...
export class MCPContextManager {
    ...
    private async detectThreadTitle(): Promise<string> {
        ...
        // Try to detect VS Code chat session ID
        const detectedSessionId = await this.detectVSCodeChatSession();
        if (detectedSessionId) {
            this.outputChannel.appendLine(`Detected VS Code chat session: ${detectedSessionId}`);
            // Use session ID as base for thread title
            const baseName = workspaceFolder?.name || 'Chat';
            return `${baseName} Session ${detectedSessionId.substring(0, 8)}`;
        }
    ...
    private async detectVSCodeChatSession(): Promise<string | null> {
        try {
            // Execute JavaScript in VS Code's webview context to detect chat session
            const sessionDetectionScript = `
            ...
            // Note: This would need proper webview execution context
            // For now, we'll return null and rely on user input
            // TODO: Implement proper VS Code chat session detection

            return null;
        } catch (error) {
            this.outputChannel.appendLine(`Chat session detection failed: ${error}`);
            return null;
        }
    }
